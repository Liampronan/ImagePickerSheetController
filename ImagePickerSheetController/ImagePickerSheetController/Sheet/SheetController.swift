...
class SheetController: NSObject {
    ...
    private(set) lazy var sheetCollectionView: UICollectionView = {
        ...
        collectionView.register(SheetPreviewCollectionViewCell.self, forCellWithReuseIdentifier: String(describing: SheetPreviewCollectionViewCell.self))
        collectionView.register(SheetActionCollectionViewCell.self, forCellWithReuseIdentifier: String(describing: SheetActionCollectionViewCell.self))
    ...
    var preferredSheetHeight: CGFloat {
        return allIndexPaths().map { self.sizeForSheetItem(at: $0).height }
                .reduce(0) { $0 + $1 }
    }
    ...
    private func allIndexPaths() -> [IndexPath] {
        ...
        return (0..<s).map { (section: Int) -> (Int, Int) in (self.numberOfItems(in: section), section) }
    ...
    private func sizeForSheetItem(at indexPath: IndexPath) -> CGSize {
        let height: CGFloat = {
            ...
            let insets = attributesForItem(at: indexPath).backgroundInsets
    ...
    private func attributesForItem(at indexPath: IndexPath) -> (corners: RoundedCorner, backgroundInsets: UIEdgeInsets) {
        ...
        if let cancelIndexPath = cancelIndexPath {
            ...
            if indexPath == indexPaths.last {
                return (RoundedCorner.bottom(cornerRadius), UIEdgeInsets(top: 0, left: sheetInset, bottom: innerInset, right: sheetInset))
            }
        } else if indexPath == indexPaths.last {
            return (RoundedCorner.bottom(cornerRadius), UIEdgeInsets(top: 0, left: sheetInset, bottom: sheetInset, right: sheetInset))
        }

        return (RoundedCorner.none, UIEdgeInsets(top: 0, left: sheetInset, bottom: 0, right: sheetInset))
    }
...
extension SheetController: UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.section == 0 {
            let previewCell = collectionView.dequeueReusableCell(withReuseIdentifier: String(describing: SheetPreviewCollectionViewCell.self), for: indexPath) as! SheetPreviewCollectionViewCell
            previewCell.collectionView = previewCollectionView

            cell = previewCell
        } else {
            let action = actions[indexPath.item]
            let actionCell = collectionView.dequeueReusableCell(withReuseIdentifier: String(describing: SheetActionCollectionViewCell.self), for: indexPath) as! SheetActionCollectionViewCell
        ...
        // iOS specific design
        (cell.roundedCorners, cell.backgroundInsets) = attributesForItem(at: indexPath)
...
extension SheetController: UICollectionViewDelegateFlowLayout {
    ...
    public func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return sizeForSheetItem(at: indexPath)
    }
